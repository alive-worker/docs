...
站内中文文章 118 篇（2026-09-20 文件盘点）：`overseas-ai-*` 仍是主力，国内大模型线已有 19 篇 `domestic-ai-*`（不再是「只有 1 篇入门导读」）。2026-08-25 的反馈仍然有效：新增时不能每次都默认写海外AI主题，要时不时选国内大模型相关的具体场景/机制（会员计费、企业API定价、实名/合规、限流、国内支付渠道等），跟海外主题交替安排。国内主题同样要满足本文件其余全部标准，资料来源改用国内官方文档/公告。

## 4. 新文章上线检查清单

生成新文章（中英文各一篇）时，除了正文本身，以下**全部**要同步完成，不要遗漏。发完后在仓库根目录跑 `node tools/verify-site.js`：它会核 DATES、sitemap、hreflang 目标文件、归档 `data-topic` 计数、专题 hub 收录，以及 `styles.min.css` / `js/site.min.js` 的 `?v=` 是否等于文件 MD5。不要把 `docs-coin` 的发布脚本拿到本站跑（本站中英双 slug、网格不封顶、minify+MD5）。
