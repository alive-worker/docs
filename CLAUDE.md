...
- **中文文章页不加品牌后缀**：不用 `{标题} - AI服务指南` 这种机械拼接格式（2026-07-14 已把全站文章标题和 H1 都改掉了旧的「- AI订阅指南」后缀）。品牌名 `AI服务指南` 只在首页 `<title>`（`AI服务指南｜{slogan}` 形式）和关于页保留，文章页标题就是标题本身，不额外拼后缀。
...
- 文章页 `<title>`、`<h1>`、`og:title`、`twitter:title`、JSON-LD `Article.headline` 与 `WebPage.name` 必须是同一字符串（按 HTML 实体解码后比较）。英文不要短 title + 长 H1 两套；超 60 字符就改短 H1。`WebPage.name` 也不加 `- AI Access Guide` / `- AI服务指南` 后缀。
...
- 新增/修改文案后，写一个 Node 脚本量长度，不要靠肉眼数字符。**HTML 属性字面量**和**实体解码后**都要落在区间内；不要用 `&mdash;` / `&quot;` 这类实体把字面量撑过上限，引号改用 `“”`。
...
站内中文文章 120 篇（2026-09-20 文件盘点）：`overseas-ai-*` 仍是主力，国内大模型线已有 20 篇 `domestic-ai-*`（不再是「只有 1 篇入门导读」）。2026-08-25 的反馈仍然有效：新增时不能每次都默认写海外AI主题，要时不时选国内大模型相关的具体场景/机制（会员计费、企业API定价、实名/合规、限流、国内支付渠道等），跟海外主题交替安排。国内主题同样要满足本文件其余全部标准，资料来源改用国内官方文档/公告。

## 4. 新文章上线检查清单

生成新文章（中英文各一篇）时，除了正文本身，以下**全部**要同步完成，不要遗漏。发完后在仓库根目录跑 `node tools/verify-site.js`：它会核 DATES、sitemap、hreflang 目标文件、归档 `data-topic` 计数、专题 hub 收录、`styles.min.css` / `js/site.min.js` 的 `?v=` 是否等于文件 MD5，以及文章页 title=H1=headline、描述字面量与解码后长度、标题无品牌后缀。不要把 `docs-coin` 的发布脚本拿到本站跑（本站中英双 slug、网格不封顶、minify+MD5）。
...
5. `js/site.js` 的 `DATES` 映射表：补充新文章的时间戳条目。**`js/site.min.js` 必须用 `npx --yes terser js/site.js -c -m -o js/site.min.js` 重新生成，禁止手工直接编辑压缩后的 `site.min.js`**——2026-09-04 出过一次真实事故：手工在压缩后的单行代码里插入两条 DATES 记录时打错了一个引号（多了个开头引号、漏了下一条的开头引号），产生了语法错误，导致整个脚本在浏览器里直接抛 `SyntaxError` 崩溃、一行都不执行——首页文章数卡在几天前的旧值、搜索框完全失灵，且没有任何显眼的报错提示给普通访客看到，光靠人工目视这种压缩成一整行的代码几乎不可能发现问题。改完 `js/site.js` 后必须用 `node -c js/site.min.js` 验证语法通过，再走一遍第8节的哈希重算流程——哪怕只加了一行数据、没碰任何逻辑代码，文件字节也已经变了，`?v=` 版本号不重算，Cloudflare 就会用旧哈希对应的缓存继续伺候好几天，线上首页统计数字之类依赖这个文件的地方会卡在旧值上（2026-08-26 已经因为漏了这步导致线上文章数連续几次发布都没更新，见第8节记录）。
